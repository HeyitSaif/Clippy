  ...
  return items.map((c) =>
    c.id === id
      ? {
          ...c,
          isSnippet,
          snippetName: isSnippet
            ? (snippetName ?? c.preview.slice(0, 40))
            : null,
        }
      : c,
  );
...
export function useClips() {
    ...
    async (opts?: { silent?: boolean }) => {
      ...
      try {
        ...
        const result = hasActiveQuery()
          ? await window.clippy.searchClips({
              ...parsed,
              limit: PAGE_SIZE,
              offset: 0,
            })
  ...
  useEffect(() => {
    // Debounce reconciliation: pin/delete/tag each broadcast CLIPS_UPDATED; coalesce bursts.
...
export function useSettings() {
  ...
  return {
    ...
    update,
    hotkeyError,
    clearHotkeyError: () => setHotkeyError(null),
  };
}
