...
import { truncateUtf8 } from "@shared/clipboard-limits";
...
function generateThumb(image: Electron.NativeImage): {
  buffer: Buffer;
  width: number;
  height: number;
} {
...
function detectFilePath(text: string): string | null {
  ...
  if (
    /^(\/Users\/|\/home\/|\/tmp\/|[A-Za-z]:\\).+/.test(trimmed) &&
    !trimmed.includes("\n")
  ) {
    return trimmed;
  }
...
function cappedString(
  ...
  const { value: next, truncated } = truncateUtf8(value);
  if (truncated) {
    log.warn(`Clipboard ${label} truncated to max UTF-8 size`);
  }
  return next;
}
...
export class ClipboardService {
  ...
  /** Cheap size+byteLength key for the last image poll (avoids sha256 when unchanged). */
  private recentImageCheapKey: string | null = null;
  ...
  private scrape(): void {
    try {
      ...
      // Tier 1: formats unchanged → skip content reads (idle).
      // Images: skip most ticks; deep-verify periodically for rare same-format replacements.
      if (key === this.lastFormatsKey && this.recentHash != null) {
        if (this.lastContentKind === "text" && !hasImage) {
          this.idleStreak++;
          return;
        }
        if (this.lastContentKind === "image" && hasImage) {
          this.idleStreak++;
          if (!shouldDeepVerifyImage(this.idleStreak)) return;
        }
      }
      ...
      if (hasImage) {
        ...
        const cheapKey = imageCheapKey(
          size.width,
          size.height,
          bitmap.byteLength,
        );

        // Cheap gate: same dimensions + byte length → skip sha256 / toPNG.
        if (
          !this.skipNext &&
          cheapKey === this.recentImageCheapKey &&
        ...
        const fingerprint = sha256(
          Buffer.concat([Buffer.from(`${cheapKey}:`), bitmap]),
        );
        if (this.skipNext) {
          this.skipNext = false;
          this.recentImageFingerprint = fingerprint;
          this.recentImageCheapKey = cheapKey;
        ...
        this.recentImageFingerprint = null;
        this.recentImageCheapKey = null;
        const textRaw = clipboard.readText();
        if (!textRaw.trim()) return;
        const text = cappedString(textRaw, "text") ?? textRaw;
        const settings = this.getSettings();
        ...
        if (!raw) {
          raw = {
            ...
            hash: contentHash,
            text,
            html: cappedString(clipboard.readHTML(), "html"),
            rtf: cappedString(clipboard.readRTF(), "rtf"),
      ...
      if (raw.type === "image" && raw.imageBuffer && raw.thumbBuffer) {
        const paths = this.clipRepo.saveImageFiles(
          raw.hash,
          raw.imageBuffer,
          raw.thumbBuffer,
        );
