...
const DEFAULT_LISTS: {
  ...
  { id: TODO_SYSTEM_LIST_IDS.inbox, name: 'Inbox', kind: 'inbox', sortOrder: 0 },
  { id: TODO_SYSTEM_LIST_IDS.daily, name: 'Daily', kind: 'daily', sortOrder: 1 },
  { id: TODO_SYSTEM_LIST_IDS.weekly, name: 'Weekly', kind: 'weekly', sortOrder: 2 }
]
...
export class TodoRepository {
  ...
  static ensureDefaults(db: Database.Database): void {
    ...
    const findById = db.prepare('SELECT id FROM todo_lists WHERE id = ? LIMIT 1')
    const findByKind = db.prepare('SELECT id FROM todo_lists WHERE kind = ? LIMIT 1')
  ...
  getListByKind(kind: TodoListKind): TodoList | null {
    const row = this.db
      .prepare('SELECT * FROM todo_lists WHERE kind = ? ORDER BY sort_order ASC LIMIT 1')
  ...
  listTodos(query: TodoFilterQuery = {}): TodoItem[] {
    ...
    if (query.q?.trim()) {
      ...
      filters.push(`(title LIKE ? ESCAPE '\\' OR IFNULL(notes, '') LIKE ? ESCAPE '\\')`)
  ...
  createTodo(input: CreateTodoInput): TodoItem {
    ...
    const id = crypto.randomUUID()
    const maxRow = this.db
      .prepare('SELECT COALESCE(MAX(sort_order), -1) AS m FROM todos WHERE list_id = ?')
  ...
  updateTodo(id: string, partial: UpdateTodoPartial): TodoItem | null {
    ...
    const priority =
      partial.priority !== undefined ? clampPriority(partial.priority) : existing.priority
    const dueAt = partial.dueAt !== undefined ? partial.dueAt : existing.dueAt
    const remindAt = partial.remindAt !== undefined ? partial.remindAt : existing.remindAt
    const sortOrder = partial.sortOrder !== undefined ? partial.sortOrder : existing.sortOrder

    let isCompleted = existing.isCompleted
    let completedAt = existing.completedAt
    if (partial.isCompleted !== undefined && partial.isCompleted !== existing.isCompleted) {
