...
const api = {
  ...
  getClip: (id: string): Promise<ClipRecord | null> =>
    ipcRenderer.invoke(IPC.CLIPS_GET, id),
  ...
  copyClip: (id: string): Promise<boolean> =>
    ipcRenderer.invoke(IPC.CLIPS_COPY, id),
  pasteClip: (id: string): Promise<boolean> =>
    ipcRenderer.invoke(IPC.CLIPS_PASTE, id),
  deleteClip: (id: string): Promise<boolean> =>
    ipcRenderer.invoke(IPC.CLIPS_DELETE, id),
  togglePin: (id: string): Promise<ClipRecord | null> =>
    ipcRenderer.invoke(IPC.CLIPS_TOGGLE_PIN, id),
  ...
  exportHistory: (): Promise<string | null> =>
    ipcRenderer.invoke(IPC.CLIPS_EXPORT),
  ...
  listTodoLists: (): Promise<TodoList[]> =>
    ipcRenderer.invoke(IPC.TODOS_LIST_LISTS),
  ...
  deleteTodo: (id: string): Promise<boolean> =>
    ipcRenderer.invoke(IPC.TODOS_DELETE, id),
  ...
  onTodoReminder: (
    cb: (payload: { todoId: string; listId: string }) => void,
  ): (() => void) => {
    const handler = (
      _: Electron.IpcRendererEvent,
      payload: { todoId: string; listId: string },
    ): void => cb(payload);
    ipcRenderer.on(IPC_EVENTS.TODO_REMINDER, handler);
    return () => ipcRenderer.removeListener(IPC_EVENTS.TODO_REMINDER, handler);
  },
  ...
  onAccessibilityRequired: (cb: () => void): (() => void) => {
    ...
    return () =>
      ipcRenderer.removeListener(IPC_EVENTS.ACCESSIBILITY_REQUIRED, handler);
  },
  ...
  onSettingsChanged: (cb: (settings: AppSettings) => void): (() => void) => {
    const handler = (_: Electron.IpcRendererEvent, s: AppSettings): void =>
      cb(s);
    ipcRenderer.on(IPC_EVENTS.SETTINGS_CHANGED, handler);
    return () =>
      ipcRenderer.removeListener(IPC_EVENTS.SETTINGS_CHANGED, handler);
  },
