...
function main () {
  ...
  const tray = new Tray(path.join(__dirname,'organize.png'));
  ...
  if(process.platform === 'darwin'){
    globalShortcut.register('Command+1', () => {
      dontSave=true
      clipboard.writeText(items.getall()[0].text);
      if (!dontPaste)
    ...
    globalShortcut.register('Command+2', () => {
      dontSave=true
      clipboard.writeText(items.getall()[1].text);
      if (!dontPaste)
      runApplescript('tell application "System Events" to keystroke "v" using command down').then().catch();
    })
    globalShortcut.register('Command+3', () => {
      dontSave=true
      clipboard.writeText(items.getall()[2].text);
      if (!dontPaste)
      runApplescript('tell application "System Events" to keystroke "v" using command down').then().catch();
    })
    globalShortcut.register('Command+4', () => {
      dontSave=true
      clipboard.writeText(items.getall()[3].text);
      if (!dontPaste)
      runApplescript('tell application "System Events" to keystroke "v" using command down').then().catch();
    })
    globalShortcut.register('Command+5', () => {
      dontSave=true
      clipboard.writeText(items.getall()[4].text);
      if (!dontPaste)
      runApplescript('tell application "System Events" to keystroke "v" using command down').then().catch();
    })
  }else{
